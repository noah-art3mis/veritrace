...
- **Spiral view (`lib/spiral-layout.ts`, `use-spiral-flow.ts`):** a **third** rendering of the same 4-layer graph, alongside Cards and Radial. An Archimedean coil from the source outward in reading order, with each question's evidence ordered by hop — so a walk reads as a continuous outward arc (depth-as-distance). It reuses the radial circle nodes, edges, and force layer; only the anchor geometry (coil vs concentric rings) differs. A run in depth mode opens on the spiral by default; the segmented Cards/Radial/Spiral control switches freely.
