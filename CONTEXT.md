...
- #2 (**Answer layer**): an explicit candidate-answer-per-Question would make the card graph 5 deep, in tension with the retired-Sub-claim rule. **ADR 0006 proposes the resolution** — derive the Answer as a stance-bucket grouping _inside_ the Question (cards) and on the evidence rim (radial), keeping the card graph 4-deep rather than adding a fifth rank. Status: _proposed_, pending ratification; flip to resolved when ADR 0006 is accepted.
- OPEN (recursion shape): the cyclic re-entrant search (ADR 0005) can surface new checkable claims while resolving a question. Whether a discovered claim joins the existing **Claims layer** (stays 4-deep — strongly preferred) or nests as a deeper **sub-claim layer** (5-deep, reviving the retired Sub-claim and colliding with ADR 0006) is unresolved. Must be settled before the re-entrant frontier lands.
